...
const detailHomeEl = document.querySelector(".detail__home");
const detailContentEl= document.querySelector(".detail__content");
const url=window.location.href
const params= new URLSearchParams(url.split("?")[1])
const contryName = params.get("name");
...
function darkMode(dark_mode) {
  if (dark_mode == "light") {
    ...
    detailHomeEl.classList.add("dark");

    headerEl.classList.remove("light");
    darkModeEl.classList.remove("light");
    detailHomeEl.classList.remove("light");
...
darkModeEl.addEventListener("click", (e) => {
  dark_mode = localStorage.getItem("darkMode") || "dark";
...
// find contry 
let contryData=contries.find((contry) => {
    return contry.name==contryName
})

console.log(contryData);

// display contry data
function displayContryContent(data){
    let html=""
    let bordersHtml=""
    // find border contries
let borderContries=contries.filter((contry) => {
    return data.borders?.includes(contry.alpha3Code)
}).map((item) => item.name)
...
borderContries.forEach(function(item) {
    ...
    let topLevelDomain=data.topLevelDomain.join(',')
    let languages=data.languages.map((lan) => lan.name).join(',')
    let currencies=data.currencies.map((cur)=> cur.name).join(',')
...
displayContryContent(contryData)
